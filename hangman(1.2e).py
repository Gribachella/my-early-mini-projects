...
def get_word(completed_words_list):
    ...
    fruits = """яблоко апельсин абрикос ананас персик арбуз груша помидор лимон инжир
    ...
    phenomenon = """сумерки заморозки иллюминация молния туман радуга заря гроза гейзер равноденствие гало
    глория мираж эхо изморозь люминесценция магнетизм статика роса""".upper().split()
    ...
    шестиугольник полумесяц эллипс куб шар конус цилиндр пирамида тор октаэдр призма тессеракт
...
def get_streak_status(points):
    ...
    elif points <= 63.5 * 1.15:
        return streak_visual[8]
    
    elif points <= 92.5 * 1.15:
        return streak_visual[9]
    elif points <= 132.5 * 1.15:
